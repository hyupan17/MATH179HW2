...
//   YOUR FUNCTIONS HERE
void histogram1(double **f, int width, int height, int N);
//void histogram2(double **f, int width, int height, int N);
...
void histogram1(double **f, int width, int height, int N) {
  int i, j;
  int *p = (int *) malloc(sizeof(int)*N); //store intensities
  for(i = 0; i < N; i++) { //for grof
    p[i] = 0;
  }

  double h = 1.0/(N-1.0); //question 1, round rof
  for(i = 0; i < width; i++) {
    for(j = 0; j < height; j++) {
      p[(int)(f[i][j]/h)]++;  //increasing the count of the intensity of the pixel
      f[i][j] = (int)(f[i][j]/h) * h;
    }
  }
  int sum = 0;
  for(i = 0; i < N; i++) {
    sum += p[i];
    p[i] = sum; //partial sum O(N)
  }
  for(i = 0; i < width; i++) {
    for(j = 0; j < height; j++) {
      f[i][j] = (double)p[(int)(f[i][j]/h)]/(width*height);
    }
  }
  free(p);
}

// double histogram2(double **f, int width, int height, int N) {
//   double min = 1.0;
//   double max = 0.0;
//   int i, j;
//   for()
